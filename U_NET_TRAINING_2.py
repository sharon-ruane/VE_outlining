...
def emb_image_batch_generator(data_folder, emb_list, batch_size, rsz, opt_z_stack_dict):
    while True:
        ...
        while len(image_section_batch) < batch_size:
            ...
            bbox1 = (emb_outlines_image.size[0]//8* 3, (emb_outlines_image.size[1]//4),
                     (emb_outlines_image.size[0]//8)*5, emb_outlines_image.size[1]/2)

            bbox2 = (emb_outlines_image.size[0]//8*3, emb_outlines_image.size[1]//2,
                    (emb_outlines_image.size[0]//8)*5, (emb_outlines_image.size[0]//4)*3)


            # draw = ImageDraw.Draw(emb_outlines_image)
            # draw.rectangle(bbox1, fill=(0,128, 0))
            # draw.rectangle(bbox2, fill=(0, 128, 255))
            # emb_outlines_image.show()

            poss_embs = [bbox1, bbox2]
            proximal_or_distal =  random.sample(poss_embs, 1)
            emb_side = proximal_or_distal[0]

            emb_raw_image = emb_raw_image.crop(box=emb_side)
            ...
            counter = 0
            # a,b = emb_outlines_image.size

            y = random.randint(60, rsz[0])
            if binary_outlines_arr.any() and \
                    y < auto_bounding_box_pixels[2] - auto_bounding_box_pixels[0] and \
                    y < auto_bounding_box_pixels[3] - auto_bounding_box_pixels[1]:
                while counter < 5 and len(image_section_batch) < batch_size:
                    crop_im, b_box = crop_image_in_special_box(emb_outlines_image, auto_bounding_box_pixels, (y, y))


                    w_range = range(b_box[0], b_box[2])
                    h_range = range(b_box[1], b_box[3])
                    bb_w1_pixels = [binary_outlines_arr[(b_box[1]-1), i] for i in w_range]
                    bb_w2_pixels = [binary_outlines_arr[(b_box[3]-1), i] for i in w_range]
                    bb_h1_pixels = [binary_outlines_arr[(i, b_box[0]-1)] for i in h_range]
                    bb_h2_pixels = [binary_outlines_arr[(i, b_box[2]-1)] for i in h_range]
